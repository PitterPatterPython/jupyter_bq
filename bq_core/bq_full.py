...
from bq_core._version import __desc__
...
class Bq(Pyodbc):
    ...
    custom_allowed_set_opts = ["bq_conn_default"]


    # These are the custom options for your integration    
    myopts = {}
    myopts['bq_max_rows'] = [1000, 'Max number of rows to return, will potentially add this to queries']
    myopts['bq_conn_default'] = ["default", 'Default instance name for connections']

    # Class Init function - Obtain a reference to the get_ipython()
    def __init__(self, shell, debug=False, *args, **kwargs):
        super(Bq, self).__init__(shell, debug=debug)
        self.debug = debug
